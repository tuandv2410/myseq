...
import { DrugCategoryDto, DrugCategoryTransDto } from '../dto/drug-category/drug-category.dto'
...
export class DrugCategoryService {
  ...
  async getAll(
    ...
    if(language){
      query.where("drugCategoryTrans.language = :language", { language: language})
    }
    try{
      const drugCategory = await query.getMany()
      this.logger.log(`User "${user.id} get drugCategories". Filters: ${JSON.stringify(filterDto)}`)
  ...
  async getById(
    ...
    if(language){
      query.andWhere("drugCategoryTrans.language = :language", { language: language})
    }
    try{
      const drugCategory = await query.getOne()
      this.logger.log(`User ${user.id} get drugCategory ${id}. Filters: ${JSON.stringify(filterDto)}`)
      if (!drugCategory) {
        throw new HttpException(`drugCategory with ID "${id}" not found`, HttpStatus.NOT_FOUND);
      }
  ...
  async getDrugReportListByCategory(
    ...
    try{
      ...
      const drugTemps = await this.drugTempRepo.find({where: {drugCategory: drugCategoryId}, relations:['drugTempTrans']})
      for(let i=0;i<drugTemps.length;i++){
        const drugReport:DrugReportEntity[] = await this.drugReportRepo.find({where: { drugTemp: drugTemps[i].id, user: userId }, relations:['genotypeDrugReports','drugReportTrans']})
        ...
        if(drugReport.length>0){
          for(let k=0;k<drugReport[0].drugReportTrans.length;k++){
            if(drugReport[0].drugReportTrans[k].language===filterDto.language){
              aTemp.drugReportTrans = {
                reportId: drugReport[0].id,
                draftConclusion: drugReport[0].drugReportTrans[k].draftConclusion,
                finalConclusion: drugReport[0].drugReportTrans[k].finalConclusion
              }
            }
          }
      ...
      throw new HttpException(`get Drug Report List By Category fail!`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getTransById(
    ...
    const query = this.drugCategoryTransRepo
      ...
      .andWhere("drug-category-trans.language = :language", { language: language })
    try{
      const drugCategoryTrans = await query.getOne()
      this.logger.log(`User ${user.id} get drugCategoryTrans by drugCategory ${id} and language ${language}`)
      if (!drugCategoryTrans) {
        throw new HttpException(`drugCategoryTrans with drugCategory ID "${id}" and language ${language} not found`, HttpStatus.NOT_FOUND);
      }
      return this.mapper.map(drugCategoryTrans, DrugCategoryTransDto)
      
    }catch(error) {
      this.logger.error(`User ${user.id} Failed to get drugCategoryTransby drugCategory ${id} and language ${language}`, error.stack);
      throw new HttpException("Failed to get drugCategoryTrans!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async create(
    ...
    try{
      ...
      if (error.code === '23505') {
        ...
        throw new HttpException("Drug Category already exists!", HttpStatus.CONFLICT);
      } else {
        this.logger.error(`User ${user.id} Failed to create drugCategory`, error.stack);
        throw new HttpException("create Drug Category fail!", HttpStatus.INTERNAL_SERVER_ERROR);
      }
    }
  }
  ...
  async update(
    ...
    try{
      ...
      for(let i=0;i<updateDrugCategoryTrans.length;i++){
        const drugCategoryTrans = await this.getTransById(id,updateDrugCategoryTrans[i].language, user)
        await this.drugCategoryTransRepo.update(drugCategoryTrans.id,updateDrugCategoryTrans[i] )
      }
      this.logger.log(`User ${user.id} update Drug Category ID : ${id}`)
      return await this.getById(id,{}, user);
    }catch (error) {
      this.logger.error(`User ${user.id} Failed to update Drug Category ID : ${id}`, error.stack);
      throw new HttpException("update Drug Category fail!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async delete(
    ...
    if (result.affected === 0) {
      ...
      throw new HttpException(`delete drug Category with ID "${id}" fail`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
