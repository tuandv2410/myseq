...
import { DrugReportDto, DrugReportTransDto } from '../dto/drug-report/drug-report.dto'
...
export class DrugReportService {
  ...
  async getAll(
    ...
    if(language){
      query.where("drugReportTrans.language = :language", { language: language})
    }
    try{
      const drugReport = await query.getMany()
      this.logger.log(`User ${user.id} get drugReports. Filters: ${JSON.stringify(filterDto)}`)
      return this.mapper.mapArray(drugReport, DrugReportDto)
    }catch(error) {
      this.logger.error(`User ${user.id} Failed to get drugReports. Filters: ${JSON.stringify(filterDto)}`, error.stack);
      throw new HttpException("Failed to get drugReports!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  async getById(
    ...
    if(language){
      query.andWhere("drugReportTrans.language = :language", { language: language})
    }
    try{
      ...
      this.logger.error(`User ${user.id} Failed to get drugReport. Filters: ${JSON.stringify(filterDto)}`, error.stack);
      throw new HttpException("Failed to get drugReport!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getTransById(
    ...
    const query = this.drugReportTransRepo
      ...
      .andWhere("drug-report-trans.language = :language", { language: language })
    try{
      const drugReportTrans = await query.getOne()
      this.logger.log(`User ${user.id} get drugReportTrans by drugReport ${id} and language ${language}`)
      if (!drugReportTrans) {
        throw new HttpException(`drugReportTrans with drugReport ID "${id}" and language ${language} not found`, HttpStatus.NOT_FOUND);
      }
      return this.mapper.map(drugReportTrans, DrugReportTransDto)
      
    }catch(error) {
      this.logger.error(`User ${user.id} Failed to get drugReportTransby drugReport ${id} and language ${language}`, error.stack);
      throw new HttpException("Failed to get drugReportTrans!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async create(
    ...
    try{
      ...
      if (error.code === '23505') {
        ...
        throw new HttpException("Drug Report already exists!", HttpStatus.CONFLICT);
      } else {
        this.logger.error(`User ${user.id} Failed to create drugReport`, error.stack);
        throw new HttpException("create Drug Report fail!", HttpStatus.INTERNAL_SERVER_ERROR);
      }
    }
  }
  ...
  async update(
    ...
    try{
      ...
      this.logger.error(`User ${user.id} Failed to update Drug Report ID : ${id}`, error.stack);
      throw new HttpException("update Drug Report fail!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async approve(
    ...
    try{
      ...
      this.logger.log(`User ${user.id} approve update conclusion of Drug Report ID : ${id}`)
      return await this.getById(userId, id,{}, user);
    }catch (error) {
      this.logger.error(`User ${user.id} Failed to approve update conclusion of Drug Report ID : ${id}`, error.stack);
      throw new HttpException("update Drug Report fail!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async delete(
    ...
    const inDb = await this.drugReportRepo.findOne({where:{id, user: userId}});
    ...
    if (result.affected === 0) {
      ...
      throw new HttpException(`delete drug Report with ID "${id}" fail`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  async createGenotype(
    ...
    try{
      ...
      for(let i=0;i<createGenotypeDrugReportTrans.length;i++){
        ...
        genotypeDrugReportTrans.geneSeq = createGenotypeDrugReportTrans[i].geneSeq
        genotypeDrugReportTrans.language = createGenotypeDrugReportTrans[i].language
        genotypeDrugReportTrans.phenotype= createGenotypeDrugReportTrans[i].phenotype
      ...
      throw new HttpException("create GenotypedrugReport fail!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getAllGenotypeDrugReport(
    ...
    const query = this.genotypeDrugReportRepo
      ...
      .leftJoinAndSelect("genotype-drug-report.genotypeDrugReportTrans","genotypeDrugReportTrans")
    if(language){
      query.where("genotypeDrugReportTrans.language = :language", { language: language})
    }
    try{
      const genotypeDrugReport = await query.getMany()
      this.logger.log(`User ${user.id} get genotypedrugReports by drugReport ${drugReportId}. Filters: ${JSON.stringify(filterDto)}`)
      return this.mapper.mapArray(genotypeDrugReport, GenotypeDrugReportDto)
    }catch(error) {
      this.logger.error(`User ${user.id} Failed to get genotypedrugReports by drugReport ${drugReportId}. Filters: ${JSON.stringify(filterDto)}`, error.stack);
      throw new HttpException("Failed to get genotypedrugReports!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
