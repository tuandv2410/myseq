...
import { ReportListDiseaseTemp, ReportListDto } from '../dto/disease-category/report-list.dto';
...
export class DiseaseCategoryService {
  ...
  async getAll(
    ...
    try{
      ...
      throw new HttpException("Failed to get diseaseCategory!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getById(
    ...
    try{
      ...
      if (!diseaseCategory) {
        throw new HttpException(`diseaseCategory with ID "${id}" not found`, HttpStatus.NOT_FOUND);
      }
  ...
  async getDiseaseReportListByCategory(
    ...
    try{
      const diseaseCategory = await this.getById(diseaseCategoryId, filterDto, user)
      let ans: ReportListDiseaseTemp[] = [];
      const diseaseTemps = await this.diseaseTempRepo.find({where: {diseaseCategory: diseaseCategoryId}, relations:['diseaseTempTrans']})
      for(let i=0;i<diseaseTemps.length;i++){
        const diseaseReport:DiseaseReportEntity[] = await this.diseaseReportRepo.find({where: { diseaseTemp: diseaseTemps[i].id, user: userId }, relations:['genotypeDiseaseReports','diseaseReportTrans']})
        let aTemp = new ReportListDiseaseTemp;
        aTemp.tempId = diseaseTemps[i].id;
        for(let j=0;j<diseaseTemps[i].diseaseTempTrans.length;j++){
          if(diseaseTemps[i].diseaseTempTrans[j].language===filterDto.language){
        ...
        if(diseaseReport.length>0){
          for(let k=0;k<diseaseReport[0].diseaseReportTrans.length;k++){
            if(diseaseReport[0].diseaseReportTrans[k].language===filterDto.language){
              aTemp.diseaseReportTrans = {
                reportId: diseaseReport[0].id,
                draftConclusion: diseaseReport[0].diseaseReportTrans[k].draftConclusion,
                finalConclusion: diseaseReport[0].diseaseReportTrans[k].finalConclusion
              }
            }
          }
      ...
      throw new HttpException(`get Disease Report List By Category fail!`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getTransById(
    ...
    try{
      ...
      if (!diseaseCategoryTrans) {
        throw new HttpException(`diseaseCategoryTrans with diseaseCategory ID "${id}" and language ${language} not found`, HttpStatus.NOT_FOUND);
      }
      ...
      throw new HttpException("Failed to get diseaseCategoryTrans!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async create(
    ...
    try{
      ...
      if (error.code === '23505') {
        ...
        throw new HttpException("Disease Category already exists!", HttpStatus.CONFLICT);
      } else {
        this.logger.error(`User ${user.id} Failed to create diseaseCategory`, error.stack);
        throw new HttpException("create Disease Category fail!", HttpStatus.INTERNAL_SERVER_ERROR);
      }
    }
  }
  ...
  async update(
    ...
    try{
      ...
      throw new HttpException("update Disease Category fail!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async delete(
    ...
    if (result.affected === 0) {
      ...
      throw new HttpException(`delete disease Category with ID "${id}" fail`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
