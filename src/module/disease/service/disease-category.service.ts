...
import { DiseaseUserReportListViewDto, genotype } from '../dto/disease-temp/disease-user-report-list-view.dto';
...
export class DiseaseCategoryService {
  ...
  constructor(
    ...
    this.mapper.createMap(GenotypeDiseaseReportEntity, genotype).forMember(
      d=>d.genotypeId,
      mapFrom(s=>s.id)
    )
  }
  ...
  async diseaseReportListByCategory(
    userId: string,
    diseaseCategoryId: string,
    user: UserDto
  ): Promise<DiseaseUserReportListViewDto[]> {
    try{
      let ans: DiseaseUserReportListViewDto[] = [];
      ...
      for(let i=0;i<diseaseTemps.length;i++){
        const diseaseReport:DiseaseReportEntity[] = await this.diseaseReportRepo.find({where: { diseaseTemp: diseaseTemps[i].id, user: userId }, relations:['genotypeDiseaseReports']})
        let aTemp = new DiseaseUserReportListViewDto;
        ...
        if(diseaseReport.length>0){
          ...
          aTemp.report = {
            ...
            genotypes: this.mapper.mapArray(diseaseReport[0].genotypeDiseaseReports,genotype),
          }
      ...
      throw new HttpException(`get genotype disease temps fail!`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
