...
import { NutritionCategoryDto, NutritionCategoryTransDto } from '../dto/nutrition-category/nutrition-category.dto'
...
export class NutritionCategoryService {
  ...
  constructor(
    ...
    private nutritionCategoryTransRepo: Repository<NutritionCategoryTransEntity>,
    ...
    this.mapper.createMap(NutritionCategoryTransEntity,NutritionCategoryTransDto)
    this.mapper.createMap(NutritionCategoryEntity,NutritionCategoryDto).forMember(
  ...
  async getAll(
    ...
    const query = this.nutritionCategoryRepo
      ...
      .leftJoinAndSelect("nutrition-category.nutritionCategoryTrans","nutritionCategoryTrans")
    if(language){
      query.where("nutritionCategoryTrans.language = :language", { language: language})
    }
    try{
      const nutritionCategory = await query.getMany()
      this.logger.log(`User "${user.id} get nutritionCategories". Filters: ${JSON.stringify(filterDto)}`)
  ...
  async getById(
    ...
    if(language){
      query.andWhere("nutritionCategoryTrans.language = :language", { language: language})
    }
    try{
      const nutritionCategory = await query.getOne()
      this.logger.log(`User ${user.id} get nutritionCategory ${id}. Filters: ${JSON.stringify(filterDto)}`)
      if (!nutritionCategory) {
        throw new HttpException(`nutritionCategory with ID "${id}" not found`, HttpStatus.NOT_FOUND);
      }
  ...
  async getNutritionReportListByCategory(
    ...
    try{
      ...
      const nutritionTemps = await this.nutritionTempRepo.find({where: {nutritionCategory: nutritionCategoryId}, relations:['nutritionTempTrans']})
      for(let i=0;i<nutritionTemps.length;i++){
        const nutritionReport:NutritionReportEntity[] = await this.nutritionReportRepo.find({where: { nutritionTemp: nutritionTemps[i].id, user: userId }, relations:['genotypeNutritionReports','nutritionReportTrans']})
        ...
        for(let j=0;j<nutritionTemps[i].nutritionTempTrans.length;j++){
          if(nutritionTemps[i].nutritionTempTrans[j].language===filterDto.language){
        ...
        if(nutritionReport.length>0){
          for(let k=0;k<nutritionReport[0].nutritionReportTrans.length;k++){
            if(nutritionReport[0].nutritionReportTrans[k].language===filterDto.language){
              aTemp.nutritionReportTrans = {
                reportId: nutritionReport[0].id,
                draftConclusion: nutritionReport[0].nutritionReportTrans[k].draftConclusion,
                finalConclusion: nutritionReport[0].nutritionReportTrans[k].finalConclusion
              }
            }
          }
          nutritionReport[0].genotypeNutritionReports.forEach(genotypeNutritionReport => {
      ...
      throw new HttpException(`get Nutrition Report List By Category fail!`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getTransById(
    ...
    const query = this.nutritionCategoryTransRepo
      ...
      .andWhere("nutrition-category-trans.language = :language", { language: language })
    try{
      const nutritionCategoryTrans = await query.getOne()
      this.logger.log(`User ${user.id} get nutritionCategoryTrans by nutritionCategory ${id} and language ${language}`)
      if (!nutritionCategoryTrans) {
        throw new HttpException(`nutritionCategoryTrans with nutritionCategory ID "${id}" and language ${language} not found`, HttpStatus.NOT_FOUND);
      }
      return this.mapper.map(nutritionCategoryTrans, NutritionCategoryTransDto)
      
    }catch(error) {
      this.logger.error(`User ${user.id} Failed to get nutritionCategoryTransby nutritionCategory ${id} and language ${language}`, error.stack);
      throw new HttpException("Failed to get nutritionCategoryTrans!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async create(
    ...
    try{
      ...
      for(let i=0;i<nutritionCategoryTrans.length;i++){
        ...
        newNutritionCategoryTrans.description = nutritionCategoryTrans[i].description
      ...
      this.logger.log(`User ${user.id} create nutritionCategory ${nutritionCategory.id}`)
      ...
      if (error.code === '23505') {
        ...
        throw new HttpException("Nutrition Category already exists!", HttpStatus.CONFLICT);
      } else {
        this.logger.error(`User ${user.id} Failed to create nutritionCategory`, error.stack);
        throw new HttpException("create Nutrition Category fail!", HttpStatus.INTERNAL_SERVER_ERROR);
      }
    }
  }
  ...
  async update(
    ...
    try{
      ...
      for(let i=0;i<updateNutritionCategoryTrans.length;i++){
        const nutritionCategoryTrans = await this.getTransById(id,updateNutritionCategoryTrans[i].language, user)
        await this.nutritionCategoryTransRepo.update(nutritionCategoryTrans.id,updateNutritionCategoryTrans[i] )
      }
      this.logger.log(`User ${user.id} update Nutrition Category ID : ${id}`)
      return await this.getById(id,{}, user);
    }catch (error) {
      this.logger.error(`User ${user.id} Failed to update Nutrition Category ID : ${id}`, error.stack);
      throw new HttpException("update Nutrition Category fail!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async delete(
    ...
    if (result.affected === 0) {
      ...
      throw new HttpException(`delete nutrition Category with ID "${id}" fail`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
