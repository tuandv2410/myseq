...
import { NutritionReportDto, NutritionReportTransDto } from '../dto/nutrition-report/nutrition-report.dto'
...
export class NutritionReportService {
  ...
  constructor(
    ...
    private genotypeNutritionReportRepo: Repository<GenotypeNutritionReportEntity>,
  ...
  async getAll(
    ...
    const query = this.nutritionReportRepo
      ...
      .leftJoinAndSelect("nutrition-report.nutritionReportTrans","nutritionReportTrans")
      .where('nutrition-report.userId = :userId', { userId: userId})
    if(language){
      query.where("nutritionReportTrans.language = :language", { language: language})
    }
    try{
      const nutritionReport = await query.getMany()
      this.logger.log(`User ${user.id} get nutritionReports. Filters: ${JSON.stringify(filterDto)}`)
      return this.mapper.mapArray(nutritionReport, NutritionReportDto)
    }catch(error) {
      this.logger.error(`User ${user.id} Failed to get nutritionReports. Filters: ${JSON.stringify(filterDto)}`, error.stack);
      throw new HttpException("Failed to get nutritionReports!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  async getById(
    ...
    if(language){
      query.andWhere("nutritionReportTrans.language = :language", { language: language})
    }
    try{
      ...
      this.logger.error(`User ${user.id} Failed to get nutritionReport. Filters: ${JSON.stringify(filterDto)}`, error.stack);
      throw new HttpException("Failed to get nutritionReport!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getTransById(
    ...
    const query = this.nutritionReportTransRepo
      ...
      .andWhere("nutrition-report-trans.language = :language", { language: language })
    try{
      const nutritionReportTrans = await query.getOne()
      this.logger.log(`User ${user.id} get nutritionReportTrans by nutritionReport ${id} and language ${language}`)
      if (!nutritionReportTrans) {
        throw new HttpException(`nutritionReportTrans with nutritionReport ID "${id}" and language ${language} not found`, HttpStatus.NOT_FOUND);
      }
      return this.mapper.map(nutritionReportTrans, NutritionReportTransDto)
      
    }catch(error) {
      this.logger.error(`User ${user.id} Failed to get nutritionReportTransby nutritionReport ${id} and language ${language}`, error.stack);
      throw new HttpException("Failed to get nutritionReportTrans!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async create(
    ...
    try{
      ...
      for(let i=0;i<nutritionReportTrans.length;i++){
        ...
        newNutritionReportTrans.draftConclusion = nutritionReportTrans[i].draftConclusion
        newNutritionReportTrans.finalConclusion = nutritionReportTrans[i].finalConclusion
        newNutritionReportTrans.nutritionReport = nutritionReport;
        
        await newNutritionReportTrans.save();
      }

      this.logger.log(`User ${user.id} create nutritionReport ${nutritionReport.id}`)
      ...
      if (error.code === '23505') {
        ...
        throw new HttpException("Nutrition Report already exists!", HttpStatus.CONFLICT);
      } else {
        this.logger.error(`User ${user.id} Failed to create nutritionReport`, error.stack);
        throw new HttpException("create Nutrition Report fail!", HttpStatus.INTERNAL_SERVER_ERROR);
      }
    }
  }
  ...
  async update(
    ...
    try{
      ...
      this.logger.error(`User ${user.id} Failed to update Nutrition Report ID : ${id}`, error.stack);
      throw new HttpException("update Nutrition Report fail!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async approve(
    ...
    try{
      ...
      this.logger.log(`User ${user.id} approve update conclusion of Nutrition Report ID : ${id}`)
      return await this.getById(userId, id,{}, user);
    }catch (error) {
      this.logger.error(`User ${user.id} Failed to approve update conclusion of Nutrition Report ID : ${id}`, error.stack);
      throw new HttpException("update Nutrition Report fail!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async delete(
    ...
    const inDb = await this.nutritionReportRepo.findOne({where:{id, user: userId}});
    ...
    if (result.affected === 0) {
      ...
      throw new HttpException(`delete nutrition Report with ID "${id}" fail`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  async createGenotype(
    ...
    try{
      ...
      const nutritionReport = await this.nutritionReportRepo.findOne(nutritionReportId)
      ...
      for(let i=0;i<createGenotypeNutritionReportTrans.length;i++){
        ...
        genotypeNutritionReportTrans.geneSeq = createGenotypeNutritionReportTrans[i].geneSeq
        genotypeNutritionReportTrans.language = createGenotypeNutritionReportTrans[i].language
        genotypeNutritionReportTrans.phenotype= createGenotypeNutritionReportTrans[i].phenotype
      ...
      throw new HttpException("create GenotypenutritionReport fail!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getAllGenotypeNutritionReport(
    ...
    const query = this.genotypeNutritionReportRepo
      ...
      .where("nutritionReport.id = :nutritionReportId", { nutritionReportId: nutritionReportId})
      .leftJoinAndSelect("genotype-nutrition-report.genotypeNutritionReportTrans","genotypeNutritionReportTrans")
    if(language){
      query.where("genotypeNutritionReportTrans.language = :language", { language: language})
    }
    try{
      const genotypeNutritionReport = await query.getMany()
      this.logger.log(`User ${user.id} get genotypenutritionReports by nutritionReport ${nutritionReportId}. Filters: ${JSON.stringify(filterDto)}`)
      return this.mapper.mapArray(genotypeNutritionReport, GenotypeNutritionReportDto)
    }catch(error) {
      this.logger.error(`User ${user.id} Failed to get genotypenutritionReports by nutritionReport ${nutritionReportId}. Filters: ${JSON.stringify(filterDto)}`, error.stack);
      throw new HttpException("Failed to get genotypenutritionReports!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
